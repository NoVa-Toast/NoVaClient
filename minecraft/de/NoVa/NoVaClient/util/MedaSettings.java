package de.NoVa.NoVaClient.util;
...
public class MedaSettings {
	...
	public void loadSettings() {
		String rawData = "{}";
		NoVaClient.getLogger().info("Attempting to load settings...");
		try {
			rawData = new String(Files.readAllBytes(Paths.get(new File(this.SETTINGS_PATH).getPath())));
		} catch (IOException e) {
			NoVaClient.getLogger().error(String.format("Could not load settings file \"%s\"", this.SETTINGS_PATH));
	...
	public void saveSettingsToFile() {
		NoVaClient.getLogger().info("Attempting to save settings...");
		
		if (!this.SETTINGS_LOADED) {
			NoVaClient.getLogger().error("Can't save settings because no settings were loaded!");
			return;
		}
	...
	public void setColorOf(String modName, String id, int color) {
		...
		try {
			...
			NoVaClient.getLogger().error("An error occured whilst trying to save settings!");
	...
	public static MedaSettings getSettings() {
		if (SETTINGS == null)
			SETTINGS = new NoVaSettings(MedaClient.Reference.resources+"/settings.json");
